...
import { Download, ArrowLeft, Search, ChevronDown, ChevronUp, Loader2, Filter } from "lucide-react";
import { getStats, getDetails, downloadUrl } from "../api/client";
import type { AnnotationResponse, AnnotationDetail, FieldMapping } from "../api/types";
...
export default function JobDetailPage() {
  ...
  const formCodes = useMemo(() => {
    const codes = Array.from(new Set(allRows.map((r) => r.form_code))).sort();
    return codes;
  }, [allRows]);
      ...
      <div className="card p-0 overflow-hidden">
        ...
        <div className="px-6 py-4 border-b border-az-border flex flex-wrap items-center gap-4">
          <div>
            ...
            <p className="text-xs" style={{ color: "#6B6B6B" }}>{detail?.total_mappings ?? "…"} total fields</p>
          </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {sorted.length === 0 ? (
                  <tr>
                    <td colSpan={6} className="px-6 py-10 text-center text-sm" style={{ color: "#6B6B6B" }}>
                  ...
                  sorted.map((row, i) => (
                    ...
                    >
                      ...
                      <td className="px-3 py-2.5">
                        <code className="text-xs font-mono font-medium" style={{ color: "#00699A" }}>{row.annotation || "—"}</code>
                      </td>
